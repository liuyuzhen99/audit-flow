...
export async function POST(
  ...
  const backendUrl = new URL(`${getBackendBaseUrl()}/v1/reviews/${reviewId}/approve`);
  ...
  try {
    ...
    if (!response.ok) {
      return createErrorResponse(
        ...
        getBackendErrorMessage(payload as never, "Failed to approve review"),
      );
    }
    ...
    return createErrorResponse(
      502,
      "review_approve_failed",
      error instanceof Error ? error.message : "Failed to reach approve review backend",
    );
  }
}
